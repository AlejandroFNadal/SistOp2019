...
class ColaListos:
    ...
    def fcfs(self):
        ...
        procesador.listos_ejecucion(None)   #LineaNueva None es el quatum, en este caso no nos interesa
    ...
    def prioridades(self,procesador):
        ...
        procesador.bloqueados_listos()  #LineaNueva
        procesador.listos_ejecucion(None)   #LineaNueva None es el quantum, en este caso no nos interesa
        procesador.imprime_cola_bloqueados() #LineaNueva
        procesador.imprime_cola_listos()    #LineaNueva
    ...
    def round_robin(self, quantum, procesador):
        ...
        if aux != None:
            ...
            if tiempo_r >0 and q >0:
                ...
                print("quantum actual : "+str(aux.get_quantum()))
                print("Tiempo restante del proceso actual : " + str(aux.get_tiempo_restante()))
                self.anade_proceso(aux) #se añade el proceso a la cola de listos
                self.imprime_cola_listos()
                procesador.set_proceso_actual(None) #Le aplicamos un expropiese venezolano
